...
kronA = np.kron(A, At)  # calculate tensor product of A and A transpose
time.sleep(time_b)
print("Result is: \n", np.round(kronA, decimals=decimal_places))

time.sleep(time_b)

# check if the tensor product of A and A transpose is invertible
if is_invertible(kronA) == True:
    print("\nThe tensor product of A and A^T is an invertible matrix of dimension", kronA.shape)
else:
    print("\nThe tensor product of A and A^T is a non-invertible matrix of dimension", kronA.shape)
...
if input("Press y for YES. Press any other key to continue without saving.\n") == 'y':
    np.savetxt('a_kron_at.txt', kronA, fmt='%.4f')
